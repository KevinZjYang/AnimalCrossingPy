...
def GetFishDeatilAndAddToSqlite(navurl):
    ...
    cursor = con.cursor()
    try:
        sql = "create table Fish (Name varchar primary key not null,Image varchar,Number integer,English varchar ,Japanese varchar,Position varchar,Sharp varchar,North varchar,South varchar,Time varchar,Price integer)"
        cursor.execute(sql)
    except:
        pass
    ...
    sql = f"insert or replace into Fish (Name,Image,Number,English ,Japanese ,Position ,Sharp ,North ,South ,Time ,Price ) values ('{sname}','{src}','{inumber}','{seng}','{sjap}','{sposition}','{ssharp}','{snorth}','{ssouth}','{stime}','{iprice}')"
    cursor.execute(sql) 
    #pprint(sql)
